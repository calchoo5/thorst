...
func _process(delta) -> void:
	...
	if !UPDATE_PLAYER_ON_PHYS_STEP:
		move_player(delta)
		rotate_player(delta)
	var thing = $Head/interactray.get_collider()
	if $Head/interactray.is_colliding():
		if (thing != null) && thing.is_in_group("interactable"):
			$gui/interacting.show()
			if thing.is_in_group("pick"):
				$gui/interacting.text = "yoink"
			if thing.is_in_group("place"):
				$gui/interacting.text = "place"
...
func move_player(delta) -> void:
	...
	if not is_on_floor():
		...
		speed = SPEED
		accel = ACCEL
	if %DialogueBox.visible && is_on_floor():
		return
	# Handle Jump.
